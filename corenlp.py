...
if os.environ.has_key("CORENLP"):
    ...
    DIRECTORY = "C:\Users\Tanshi Pradhan\Desktop\stanford-corenlp-full-2018-02-27\\"
...
def extract_version(corenlp_path, prefix):
    for f in os.listdir(corenlp_path):
        m = re.match("%s-([^-]*)\.jar" % prefix, f)
        if m:
            return m.groups()[0]
...
def init_corenlp_command(corenlp_path, memory, properties):
    ...
    """
    nlp_version = extract_version(corenlp_path, "stanford-corenlp")
    ejml_version = extract_version(corenlp_path, "ejml")
    jars = ["stanford-corenlp-%s.jar" % nlp_version,
            "stanford-corenlp-%s-models.jar" % nlp_version,
            "xom.jar",
            "joda-time.jar",
            "jollyday.jar",
            "ejml-%s.jar" % ejml_version]
...
def parse_parser_xml_results(xml, file_name="", raw_output=False):
    ...
    def enforceList(list_or_ordered_dict):
...
class StanfordCoreNLP:
    ...
    def _spawn_corenlp(self):
        ...
        self.corenlp = pexpect.popen_spawn.PopenSpawn(self.start_corenlp, timeout=60, maxread=8192, searchwindowsize=80)
